...
From the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-project[Topology View for your project^], click on the `+` icon in the header menu, and choose "Container Image"
...
On the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-project[Topology View for your project^], you'll see prometheus spinning up.
...
Once the build completes, ensure the app completes its redeployment with this command (or watch the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-project[Topology View for your project^])
...
You'll need to trigger the methods that we've instrumented, so https://people-{user}-project.{openshift_cluster_ingress_domain}/names.html[reopen the name cloud^], which will start producing names (and generating metrics):

image::names.png[names, 400]

Within about 15-30 seconds, Prometheus should start scraping the metrics. Once again, access the http://prometheus-{user}-project.{openshift_cluster_ingress_domain}[Prometheus UI^]. Start typing in the query box to look for metrics that begin with 'name':
...
Follow the same process as before: On the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-project[Topology View^], select the `+` icon in the header menu, and choose "Container Image".
...
On the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-project[Topology View for your project^], you'll see Grafana spinning up. Once it completes, select on the arrow to access the Grafana UI:
...
* *URL*: `http://prometheus.{user}-project:9090`
...
Back on the http://grafana-{user}-project.{openshift_cluster_ingress_domain}[Grafana Home^], select *New Dashboard* to create a new _Dashboard_ to review the metrics.
