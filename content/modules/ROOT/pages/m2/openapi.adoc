...
image::weblink.png[swagger,700, link=self,window=blank]

Then, click on `Open`.

image::weblink-open.png[swagger,700, link=self,window=blank]

Then, add `/q/swagger-ui` to the end of the URL to access the Swagger UI and play with your API. Be sure to use *http* protocol to access the Swagger UI page.

image::swagger.png[swagger,700, link=self,window=blank]

Using the UI, expand the `/person/birth/before/\{year\}` endpoint. Here you can see basic detail about the endpoint: the name of the endpoint, parameters and their type, and the response type one can expect.

image::swaggeryear.png[swaggeryear,900, link=self,window=blank]

Select the **Try it out** button to expand the box allowing you to try it. Enter the year `1990` in the box and select **Execute**:

image::swaggeryeartest.png[swaggeryeartest,900, link=self,window=blank]
...
image::swaggeryeartestresult.png[swaggeryeartestresult,900, link=self,window=blank]
...
. Augment your JAX-RS annotations with the OpenAPI https://github.com/eclipse/microprofile-open-api/blob/main/spec/src/main/asciidoc/microprofile-openapi-spec.asciidoc#annotations[Annotations^]. Using annotations means you don't have to re-write the portions of the OpenAPI document that are already covered by the JAX-RS framework (e.g. the HTTP method of an operation).
. Take the initial output from `/openapi` as a starting point to document your APIs via https://github.com/eclipse/microprofile-open-api/blob/main/spec/src/main/asciidoc/microprofile-openapi-spec.asciidoc#static-openapi-files[Static OpenAPI files^] (and then check those into source code repositories and update when APIs change). It's worth mentioning that these static files can also be written before any code, which is an approach often adopted by enterprises that want to lock-in the contract of the API. In this case, we refer to the OpenAPI document as the "source of truth", by which the client and provider must abide.
. Use the https://github.com/eclipse/microprofile-open-api/blob/main/spec/src/main/asciidoc/microprofile-openapi-spec.asciidoc#programming-model[Programming model^] to provide a bootstrap (or complete) OpenAPI model tree.

Additionally, a https://github.com/eclipse/microprofile-open-api/blob/main/spec/src/main/asciidoc/microprofile-openapi-spec.asciidoc#filter[Filter^] is described which can update the OpenAPI model after it has been built from the previously described documentation mechanisms.


=== Add OpenAPI annotations

So let's add a bit more documentation to our `/person/birth/before/\{year\}` endpoint using the first option (annotations). 

Open the `PersonResource` class, and find the `getBeforeYear` method -- this method implements our endpoint.
...
image::swaggerparams.png[swaggerparams,900, link=self,window=blank]
