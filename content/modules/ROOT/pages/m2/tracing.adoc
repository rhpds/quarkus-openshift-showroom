...
https://grafana.com/docs/tempo/latest/[Tempo^] is a distributed tracing system created by Grafana Labs. It is used for monitoring and troubleshooting microservices-based distributed systems, including:
...
You'll use Tempo as a backend to collect the telemetry data because Tempo exposes a _collector_ through which apps (like our People app) report tracing details. Tempo stores and reports on this tracing activity.
...
Tempo is installed and managed through its Kubernetes Operator (just like AMQ Streams and Kafka that you used in previous exercises).

Under **Ecosystem** in the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-quarkus[sidebar menu^], click on **Installed Operators** and then select **TempoMonolithic**

image::operators-list.png[operators,800, link=self,window=blank]

Select the **Current namespace only** option before clicking on the **Create TempoMonolithic** button:

image::jaegers-list.png[jaegers,800, link=self,window=blank]

This will open a dialog for you to configure the Tempo service before it's installed.

Change the input type to YAML, then paste in the following:
...
Click the *Create* button to deploy your new `TempoMonolithic` instance.

This will create a new `TempoMonolithic` Kubernetes object in your namespace, triggering the Operator to deploy Tempo. In the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-quarkus[Topology View^] you'll see Tempo spin up:

image::jaegerspin.png[spin, 600, link=self,window=blank]

Tempo exposes its data at different ports for different protocols. Most interactions involve the HTTP collector at `tempo-tempo:3200` but other protocols like gRPC are also supported on different ports. You can see them by clicking on the https://console-openshift-console.{openshift_cluster_ingress_domain}/k8s/ns/{user}-quarkus/services/tempo-tempo[Service details^]:

image::jaegersvcs.png[spin, 700, link=self,window=blank]

Your app will use the otlp endpoints on ports `4317` and `4318` to interact with OpenTelemetry in the next section.

== Install OpenTelemetry Collector

OpenTelemetry Collector enables you to offload data quickly alongside your application services in terms of retries, batching, encryption or even sensitive data filtering. You will create an OpenTelemetry Collector to send the telemetry data to the Tempo server.

View the `otel.yml` in the *src/main/kubernetes* directory.
...
Then, run the following `oc` command in the VS Code terminal to setup your otel-collector:
...
image::devconsole-otel.png[devconsole-otel,800, link=self,window=blank]

== Add OpenTelemetry to Quarkus

With Tempo installed, let's turn back to our Quarkus app. Like other exercises, we'll need the following extensions to enable OpenTelemetry in our app. Install it with:

[source,sh,role="copypaste"]
----
mvn quarkus:add-extension -Dextensions="quarkus-opentelemetry, rest-client, quarkus-rest-client-jackson"
----
...
[INFO] [SUCCESS] ✅  Extension io.quarkus:quarkus-opentelemetry has been installed
[INFO] [SUCCESS] ✅  Extension io.quarkus:quarkus-rest-client-jackson has been installed
...
Next, open the `application.properties` file (in the `src/main/resources` directory). Add the following lines to it to configure Tempo to send spans via the OTLP gRPC Exporter in Quarkus:
...
You'll need to trigger some HTTP endpoints to generate traces. Access the https://people-{user}-quarkus.{openshift_cluster_ingress_domain}/datatable.html[graphical person browser powered by the DataTables library we created earlier^].
...
image::paging.png[paging,600, link=self,window=blank]

== Inspect traces

Open the https://tempo-tempo-jaegerui-{user}-quarkus.{openshift_cluster_ingress_domain}[Jaeger Query UI^]. By default, Tempo offers a Jaeger UI that uses the same login details as OpenShift, so click the **Login with OpenShift** button and enter your credentials:

* **Username**: `{user}`
* **Password**: `{password}`

Then, click **Allow Selected Permissions** to allow Tempo to access your account details. You'll end up on the Jaeger query page. Using the menu on the left, select the `people` Service, and click **Find Traces**. Jaeger will show the collected traces on the right:

image::find1.png[tempo,600, link=self,window=blank]

Select one of the traces from "a few seconds ago" to show the individual _spans_ of each trace:

image::trace1.png[tempo,600, link=self,window=blank]
...
This exercise shows how to use the https://github.com/eclipse/microprofile-rest-client[MicroProfile REST Client^] with Quarkus in order to trace _external_, outbound requests with very little effort.

We will use the publicly available https://swapi.info[Star Wars API^] to fetch some characters from the Star Wars universe. Our first order of business is to setup the model we will be using, in the form of a StarWarsPerson POJO.
...
Having this configuration means that all requests performed using our code will use `https://swapi.info` as the base URL.
...
Using the configuration above, calling the `getPerson(int)` method of StarWarsService with a value of `1` would result in an HTTP GET request being made to `https://swapi.info/api/people/1/`. Confirm you can access the Star Wars API using curl:

[source,sh,role="copypaste"]
----
curl -s https://swapi.info/api/people/1 | jq
----
...
Reload the https://tempo-tempo-jaegerui-{user}-quarkus.{openshift_cluster_ingress_domain}[Jaeger Query UI^]. Then, select `GET /person/swpeople` in the Operation and click **Find Traces**.

image::swpeople-search.png[swpeople,800, link=self,window=blank]

The new trace should appear the top with multiple spans. Select it to display details:

image::swpeople.png[swpeople,800, link=self,window=blank]
...
image::swpeopleext.png[swpeopleext,800, link=self,window=blank]
...
You've seen how to enable automatic tracing for JAX-RS methods as well as create custom tracers for non-JAX-RS methods and external services by using OpenTelemetry and Tempo. This specification makes it easy for Quarkus developers to instrument services with distributed tracing for learning, debugging, performance tuning, and general analysis of behavior.
