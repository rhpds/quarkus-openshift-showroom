...
From the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-project[Topology View^], click on **Installed Operators** in the sidebar menu and select **Tempo**
...
You'll need to trigger some HTTP endpoints to generate traces. Access the https://people-{user}-project.{openshift_cluster_ingress_domain}/datatable.html[graphical person browser powered by the DataTables library we created earlier^].
...
Open the https://tempo-{user}-project.{openshift_cluster_ingress_domain}[Jaeger Query UI^]. By default, Tempo offers a Jaeger UI that uses the same login details as OpenShift, so click the **Login with OpenShift** button and enter your credentials:
...
Reload the https://tempo-{user}-project.{openshift_cluster_ingress_domain}[Jaeger Query UI^]. Then, select `GET /person/swpeople` in the Operation and click **Find Traces**.
