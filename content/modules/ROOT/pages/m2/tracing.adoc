...
Tempo exposes its collector at different ports for different protocols. Most use the HTTP collector at `tempo-collector:14268` but other protocols like gRPC are also supported on different ports. You can see them by clicking on the https://console-openshift-console.{openshift_cluster_ingress_domain}/k8s/ns/{user}-quarkus/services/tempo-collector[Service details^]:
