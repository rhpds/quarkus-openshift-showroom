...
From the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-project[Topology View^], click on **Installed Operators** in the sidebar menu and select **Jaeger**
...
Open the https://jaeger-{user}-project.{openshift_cluster_ingress_domain}[Jaeger Query UI^]. By default Jaeger uses the same login details as OpenShift, so click the **Login with OpenShift** button, enter your credentials:
...
Reload the https://jaeger-{user}-project.{openshift_cluster_ingress_domain}[Jaeger Query UI^]. Then, select `GET /person/swpeople` in the Operation and click **Find Traces**.
