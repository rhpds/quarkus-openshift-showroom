...
From the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-quarkus[Topology View^], click on **Installed Operators** in the sidebar menu and select **Jaeger**

image::operators-list.png[operators,800, link=self,window=blank]

Select **Create Jaeger**:
...
image::jaegerdone.png[kafkacatalog,800, link=self,window=blank]

This will create a new `Jaeger` Kubernetes object in your namespace, triggering the Operator to deploy Jaeger. In the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-quarkus[Topology View^] you'll see Jaeger spin up:

image::jaegerspin.png[spin, 600, link=self,window=blank]

Jaeger exposes its collector at different ports for different protocols. Most use the HTTP collector at `jaeger-collector:14268` but other protocols like gRPC are also supported on different ports. You can see them by clicking on the https://console-openshift-console.{openshift_cluster_ingress_domain}/k8s/ns/{user}-quarkus/services/jaeger-collector[Service details^]:
...
Open the https://jaeger-{user}-quarkus.{openshift_cluster_ingress_domain}[Jaeger Query UI^]. By default Jaeger uses the same login details as OpenShift, so click the **Login with OpenShift** button, enter your credentials:
...
image::find1.png[jaeger,600, link=self,window=blank]

Select one of the traces from "a few seconds ago" to show the individual _spans_ of each trace:

image::trace1.png[jaeger,600, link=self,window=blank]
...
Reload the https://jaeger-{user}-quarkus.{openshift_cluster_ingress_domain}[Jaeger Query UI^]. Then, select `GET /person/swpeople` in the Operation and click **Find Traces**.
