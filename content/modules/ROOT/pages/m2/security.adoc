...
Keycloak needs a database to store its data. We'll use Postgres. Back on the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-quarkus?view=graph[OpenShift web console^], select **Software Catalog**, under **Home** on the left.

image::db.png[db, 300, link=self,window=blank]

Then, Type in `PostgreSQL (Ephemeral)` in the search box, and select the *PostgreSQL (ephemeral)*:

image::db-postgres.png[postrgesdb, 700, link=self,window=blank]

Select *Instantiate Template*, then override the following values (leaving the rest as their defaults):
...
image::postgres-create2-kc.png[kc, 800, link=self,window=blank]

You should see a postgres database begin spinning up:

image::postgres-create3-kc.png[kc, 800, link=self,window=blank]
...
----
oc -n {user}-quarkus label dc keycloak-postgresql "app.openshift.io/runtime=postgresql" --overwrite
----
...
openssl req -subj '/CN={openshift_cluster_ingress_domain}/O=Test Keycloak/C=US' -newkey rsa:2048 -nodes -keyout key.pem -x509 -days 365 -out $PROJECT_SOURCE/src/main/resources/certs/certificate.pem && \
oc -n {user}-quarkus create secret tls keycloak-cert-secret --cert $PROJECT_SOURCE/src/main/resources/certs/certificate.pem --key key.pem
----
This secret will be used in the next step.

=== Create Keycloak instance

By creating an instance of a `Keycloak` object, the Keycloak operator will process it and instantiate a new Keycloak server. 

. In the OpenShift web console, click on **Ecosystem > Installed Operators** in the sidebar menu.

. Then type `keycloak` into the search box, click on the "Keycloak" item, and click Create:
+
image::keycloak-create1.png[kc, 800, link=self,window=blank]

. Choose the *Create Keycloak* button
+
image::keycloak-create-btn.png[kc, 800, link=self,window=blank]
...
spec:
  ...
  hostname:
    hostname: keycloak-{user}-quarkus.{openshift_cluster_ingress_domain}
  db:
    vendor: postgres
...
. Click *Create* to establish this custom resource.
...
+
image::keycloak-create2.png[kc, 500, link=self,window=blank]
...
[source,sh,role="copypaste",subs="attributes"]
----
oc -n {user}-quarkus annotate statefulset keycloak "app.openshift.io/connects-to=keycloak-postgresql" --overwrite
----
...
image::keycloak-secrets1.png[kc, 800, link=self,window=blank]

Finally, click on *Reveal Values* to see the username (which should be `admin`), and the password:

image::keycloak-adminsecret.png[kc, 800, link=self,window=blank]
...
Back on the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-quarkus?view=graph[OpenShift web console^] topology page, click the small "open" icon at the upper right of the Keycloak deployment:

image::keycloak-openicon.png[kc, 500, link=self,window=blank]

Log in with the following:

* *Username or email*: `admin`
* *Password*: The password you retrieved from the https://console-openshift-console.{openshift_cluster_ingress_domain}/k8s/ns/{user}-quarkus/secrets/keycloak-initial-admin[secret^] earlier.

image::admin-pw.png[kc, 800, link=self,window=blank]

Once logged in, you should be at the Admin home screen:

image::admin-home.png[kc, 800, link=self,window=blank]
...
For authentication purposes, we'll import a new realm called `quarkus` that has multiple users that we can test our authenticated endpoint code with, as well as a "confidential resource" based on access path, which will enable Keycloak to enforce authorization to this resource without code change.
...
We have pre-defined a new realm for this lab, in the `inventory/src/main/resources/kc-realm.yaml` file. The file contains a definition of the realm, which you can see by opening the file in DevSpaces:

image::realm-file.png[kc, 800, link=self,window=blank]

Run the following command to create this object and cause the Keycloak operator to import the realm via a `Job`:

[source,sh,role="copypaste",subs="attributes"]
----
oc apply -n {user}-quarkus -f $PROJECT_SOURCE/src/main/resources/kc-realm.yaml
----
...
image::realm-job.png[kc, 500, link=self,window=blank]

=== Explore Realm

Go back to the https://keycloak-{user}-quarkus.{openshift_cluster_ingress_domain}/[Keycloak administration console^] and login as `admin` user (using the password from the https://console-openshift-console.{openshift_cluster_ingress_domain}/k8s/ns/{user}-quarkus/secrets/keycloak-initial-admin[Secret^] as before). You should now see a new realm in the Realm dropdown. Select the `quarkus` realm by clicking on it.

Within the new realm, click on `Users` to see the pre-created users `admin`, `alice`, and `jdoe`:

image::new-users.png[kc, 800, link=self,window=blank]
...
image::new-client.png[kc, 800, link=self,window=blank]
...
----
mp.jwt.verify.publickey.location=https://keycloak-{user}-quarkus.{openshift_cluster_ingress_domain}/realms/quarkus/protocol/openid-connect/certs
mp.jwt.verify.issuer=https://keycloak-{user}-quarkus.{openshift_cluster_ingress_domain}/realms/quarkus
...
[source,sh,role="copypaste"]
----
curl -i https://$(oc get route people -o=go-template --template='{{ .spec.host }}')/secured/me
----
...
----
export ALICE_TOKEN=$(\
    curl -ks -X POST https://keycloak-{user}-quarkus.{openshift_cluster_ingress_domain}/realms/quarkus/protocol/openid-connect/token \
...
[source,sh,role="copypaste"]
----
curl -i https://$(oc get route people -o=go-template --template='{{ .spec.host }}')/secured/me/admin \
  -H "Authorization: Bearer $ALICE_TOKEN"
----
...
Injected issuer: ["https://keycloak-{user}-quarkus.{openshift_cluster_ingress_domain}/realms/quarkus"]
...
[source,none,role="copypaste",subs="attributes"]
----
# OIDC config
quarkus.oidc.auth-server-url=https://keycloak-{user}-quarkus.{openshift_cluster_ingress_domain}/realms/quarkus
...
----
curl -i -X GET \
  https://$(oc get route people -o=go-template --template='{{ .spec.host }}')/secured/confidential \
  -H "Authorization: Bearer $ADMIN_TOKEN"
----
