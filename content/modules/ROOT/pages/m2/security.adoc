...
Keycloak needs a database to store its data. We'll use Postgres. Back on the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-project?view=graph[OpenShift web console^], select **Software Catalog**, under **Home** on the left.

image::db.png[db, 300]

Then, Type in `PostgreSQL (Ephemeral)` in the search box, and select the *PostgreSQL (ephemeral)*:

image::db-postgres.png[postrgesdb, 700]
...
----
oc -n {user}-project label dc keycloak-postgresql "app.openshift.io/runtime=postgresql" --overwrite
----
...
oc -n {user}-project create secret tls keycloak-cert-secret --cert $PROJECT_SOURCE/src/main/resources/certs/certificate.pem --key key.pem
----
...
spec:
  ...
  hostname:
    hostname: keycloak-{user}-project.{openshift_cluster_ingress_domain}
  db:
    vendor: postgres
...
[source,sh,role="copypaste",subs="attributes"]
----
oc -n {user}-project annotate statefulset keycloak "app.openshift.io/connects-to=keycloak-postgresql" --overwrite
----
...
Back on the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-project?view=graph[OpenShift web console^] topology page, click the small "open" icon at the upper right of the Keycloak deployment:
...
* *Password*: The password you retrieved from the https://console-openshift-console.{openshift_cluster_ingress_domain}/k8s/ns/{user}-project/secrets/keycloak-initial-admin[secret^] earlier.
...
[source,sh,role="copypaste",subs="attributes"]
----
oc apply -n {user}-project -f $PROJECT_SOURCE/src/main/resources/kc-realm.yaml
----
...
Go back to the https://keycloak-{user}-project.{openshift_cluster_ingress_domain}/[Keycloak administration console^] and login as `admin` user (using the password from the https://console-openshift-console.{openshift_cluster_ingress_domain}/k8s/ns/{user}-project/secrets/keycloak-initial-admin[Secret^] as before). You should now see a new realm in the Realm dropdown. Select the `quarkus` realm by clicking on it.
...
----
mp.jwt.verify.publickey.location=https://keycloak-{user}-project.{openshift_cluster_ingress_domain}/realms/quarkus/protocol/openid-connect/certs
mp.jwt.verify.issuer=https://keycloak-{user}-project.{openshift_cluster_ingress_domain}/realms/quarkus
...
----
export ALICE_TOKEN=$(\
    curl -ks -X POST https://keycloak-{user}-project.{openshift_cluster_ingress_domain}/realms/quarkus/protocol/openid-connect/token \
...
Injected issuer: ["https://keycloak-{user}-project.{openshift_cluster_ingress_domain}/realms/quarkus"]
...
----
# OIDC config
quarkus.oidc.auth-server-url=https://keycloak-{user}-project.{openshift_cluster_ingress_domain}/realms/quarkus
