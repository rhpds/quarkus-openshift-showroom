...
Keycloak needs a database to store its data. We'll use Postgres. Back on the https://console-openshift-console.{ROUTE_SUBDOMAIN}/topology/ns/{USER_ID}-project?view=graph[OpenShift web console^], select **Software Catalog**, under **Home** on the left.
...
Back on the https://console-openshift-console.{ROUTE_SUBDOMAIN}/topology/ns/{USER_ID}-project?view=graph[OpenShift web console^] topology page, click the small "open" icon at the upper right of the Keycloak deployment:
...
* *Password*: The password you retrieved from the https://console-openshift-console.{ROUTE_SUBDOMAIN}/k8s/ns/{USER_ID}-project/secrets/keycloak-initial-admin[secret^] earlier.
...
----
mp.jwt.verify.publickey.location=https://keycloak-{USER_ID}-project.{ROUTE_SUBDOMAIN}/realms/quarkus/protocol/openid-connect/certs
mp.jwt.verify.issuer=https://keycloak-{USER_ID}-project.{ROUTE_SUBDOMAIN}/realms/quarkus
