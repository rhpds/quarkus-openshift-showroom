...
Our production environment needs a "real" database so let's deploy a Postgres database to OpenShift. Back in the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-quarkus[Topology View^], select **Software Catalog**, under **Ecosystem** on the left.
...
Now access the http://people-{user}-quarkus.{openshift_cluster_ingress_domain}/datatable.html[graphical person browser powered by the DataTables library^]. It should look like:
