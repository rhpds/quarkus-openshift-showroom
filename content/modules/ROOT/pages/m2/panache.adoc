...
Our production environment needs a "real" database so let's deploy a Postgres database to OpenShift. Back in the https://console-openshift-console.{ROUTE_SUBDOMAIN}/topology/ns/{USER_ID}-project[Topology View^], select **Software Catalog**, under **Home** on the left.
...
This will deploy the database to our new project. Select the https://console-openshift-console.{ROUTE_SUBDOMAIN}/topology/ns/{USER_ID}-project[Topology View^] to see it:
...
Back on the https://console-openshift-console.{ROUTE_SUBDOMAIN}/topology/ns/{USER_ID}-project[Topology View^], make sure it's done deploying (dark blue circle):
