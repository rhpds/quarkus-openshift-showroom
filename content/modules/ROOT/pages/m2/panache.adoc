...
. From the web IDE, access the file _src/main/resources/application.properties_, to see the following configuration which has been already added in advance.
+
[source,properties]
----
...
----
+
NOTE: Find more information about the Quarkus Dev Services https://quarkus.io/guides/databases-dev-services[here^].

. You will also notice the following logs in the *Quarkus Dev Mode* terminal specifying that the Dev Services for PostgreSQL has started:
+
[source,console]
----
...
. Execute `podman ps` in the bash terminal. You will see that a *PostgreSQL* container is running automatically.
+
[source,console]
----
CONTAINER ID  IMAGE                  COMMAND     CREATED         STATUS        PORTS       NAMES
27ee26a42fae  docker.io/postgres:17              24 seconds ago  Healthy (Up)              27ee26a42faebacacfcccacde227e210b6884f66ffd7dcf13f4470c56ba7a8e7
----
+
[IMPORTANT]
====
If Quarkus fails to pull the PostgreSQL image due to the pull limit in docker.io, add the following configuration to your `application.properties` file:

[source, properties,role="copypaste"]
----
%dev.quarkus.datasource.devservices.image-name=quay.io/danieloh30/postgres:14
----
====
...
We'll first need the definition of eye colors, so let's create an `enum`. 

. Create a new java class file in `src/main/java/org/acme/people/model` called `EyeColor.java`, and replace the following enum definition:
+
...
. As you can see we've defined three fields `name`, `birth`, and `eyes`. We're using the Java Persistence APIs `@Enumerated` field type for eye color.

== Define RESTful endpoint

Next, we'll create a `PersonResource` class which we will use for our RESTful endpoint. 

. Create a new Java class in the `src/main/java/org/acme/people/rest` directory called `PersonResource.java` with the following code:
+
[source,java,role="copypaste"]
----
...
Let's add some sample data to the database so we can test things out. 

. Create a new file `src/main/resources/import.sql` and add some SQL statements to the file to run on startup.
...
With the app running, let's try out our first RESTful endpoint to retrieve all the sample users. 

. Open up a separate Terminal and issue the following command:
+
[source,sh,role="copypaste"]
----
...
----
+
We call the endpoint with `curl` then send the output through `jq` to make the output prettier. 
+
You should see:
+
[source,json]
...
. Open the `Person` entity class (it's in the `org.acme.person.model` package), and add the following code under the `// TODO: Add more queries` comment:
+
[source,java,role="copypaste"]
----
...
. In our `PersonResource` endpoint, we'll start with an empty `result` object using the pre-created `DataTable` model. Add this code below the `// TODO: Begin Result` comment:
+
[source,java,role="copypaste"]
----
...
. Add this code below the `// TODO: Filter based on search` marker:
+
[source,java,role="copypaste"]
----
...
. Add this code below the `// TODO: Page and return` marker:
+
[source,java,role="copypaste"]
----
...
----
+
NOTE: The `data`, `draw`, `recordsFiltered` and `recordsTotal` values are what the DataTables frontend will be expecting when it calls this endpoint.
...
Although our lifecycle code is listening for `StartupEvent`, and our application has already started, in `quarkus:dev` mode Quarkus will still fire this event once. So let's test it out and see if it picks up our new data. 

. We'll search for a single letter `F` and limit the results to `2`:
+
[source,sh,role="copypaste"]
----
...
. Access the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-quarkus[OpenShift console^]. If needed, login *{user} / {password}*

. Select **Software Catalog**, under **Ecosystem** on the left.
+
image::db.png[db, 300, link=self,window=blank]

. Type in `PostgreSQL (Ephemeral)` in the search box, and select the *PostgreSQL (ephemeral)*:
+
...
+
* **Namespace** (the first one): Select `{user}-quarkus`
* **Memory Limit** : Keep the default `512Mi`.
...
+
image::db-postgres-values.png[db, 700, link=self,window=blank]

. Select *Create*, and run the following commans in the web IDE to label the items with proper icons:
+
[source,sh,role="copypaste"]
----
...
. This will deploy the database to our new project. Select the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-quarkus?view=graph[Topology View^] to see it:
+
image::database-deployment.png[inventory_db_deployments, 700, link=self,window=blank]

== Rebuild and redeploy app

In previous steps we deployed our sample application as a native binary. Now let's switch to a JVM-based deployment from here onwards.
...
. Back on the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-quarkus[Topology View^], make sure it's done deploying (dark blue circle):
+
[NOTE]
====
...
====
+
image::database-redeployment.png[inventory_db_deployments, 700, link=self,window=blank]

== Access deployed app

Now that we have our app running on OpenShift, let's see what we can do.

. Access the app using `curl` once again to find everyone born in or before the year 2000 (there will be many).
+
[source,sh,role="copypaste"]
----
...
. Next, access the http://people-{user}-quarkus.{openshift_cluster_ingress_domain}/datatable.html[graphical person browser powered by the DataTables library^]. It should look like:
+
image::datatable.png[datatable,800, link=self,window=blank]
