...
Our production environment needs a "real" database so let's deploy a Postgres database to OpenShift. Back in the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-project[Topology View^], select **Software Catalog**, under **Home** on the left.
...
* **Namespace** (the first one): Select `{user}-project`
...
This will deploy the database to our new project. Select the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-project[Topology View^] to see it:
...
Back on the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{user}-project[Topology View^], make sure it's done deploying (dark blue circle):
...
Now access the http://people-{user}-project.{openshift_cluster_ingress_domain}/datatable.html[graphical person browser powered by the DataTables library^]. It should look like:
