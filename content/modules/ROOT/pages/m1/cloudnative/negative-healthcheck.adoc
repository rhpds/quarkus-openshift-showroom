...
. Since we are running Quarkus dev mode, add this to to the end of the `src/main/resources/application.properties` file:
+
...
. Access the health check again using the same `curl` command - it should be `UP`!
+
[source, sh, role="copypaste"]
----
curl -i http://localhost:8080/q/health/live
